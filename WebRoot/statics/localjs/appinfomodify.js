...
function delfile(id){
	$.ajax({
		...
		success:function(data){//data：返回数据（json对象）
			if(data.result== "true"){
				alert("删除成功！");
				$("#uploadfile").html("<input id='attach'  type='file' class='form-control col-md-7 col-xs-12' name='attach' required='required'/>"+$("#uploadfile").html());
				$("#uploadfile").show();
				$("#logoFile").html('');
...
$(function(){  
  ...
  $("#updateStatus").on("click",function(){
	$("#form").attr("action",$("#form").attr("action")+"?isUpdateStatus='1'");
	$("#form").submit();
  });
});
