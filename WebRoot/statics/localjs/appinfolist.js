$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"app/categoryList",//请求的url
			data:{id:queryCategoryLevel1},//请求参数 
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				$("#queryCategoryLevel2").html("");
				var options = "<option value=\"0\">&emsp;&emsp;&emsp;&emsp;---请选择--&emsp;&emsp;&emsp;&emsp;</option>";
				
				for(var i = 0; i < data.length; i++){	
					if($("#categorylevel2Select").val()!= null && $("#categorylevel2Select").val() != undefined && data[i].id == $("#categorylevel2Select").val()){
						options += "<option selected=\"selected\" value=\""+data[i].id+"\" >"+data[i].categoryName+"</option>";
						$("#queryCategoryLevel2").val(data[i].id);
					}else{
...
$("#queryCategoryLevel2").change(function(){
	...
	if(queryCategoryLevel2 != '' && queryCategoryLevel2 != null){
		$.ajax({
			...
			success:function(data){//data：返回数据（json对象）
				...
				for(var i = 0; i < data.length; i++){
					...
					if($("#categorylevel3Select").val()!= null && $("#categorylevel3Select").val() != undefined && data[i].id == $("#categorylevel3Select").val()){
						options += "<option selected=\"selected\" value=\""+data[i].id+"\" >"+data[i].categoryName+"</option>";
					}else{
						options += "<option value=\""+data[i].id+"\">"+data[i].categoryName+"</option>";
					}
		...
		$("#queryCategoryLevel3").html("");
		var options = "<option value=\"\">&emsp;&emsp;&emsp;&emsp;---请选择--&emsp;&emsp;&emsp;&emsp;</option>";
		$("#queryCategoryLevel3").html(options);
	}
});
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="app/toAddVersion?id="+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="app/toUpdateVersion?aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="app/updateInfo?id="+ obj.attr("appinfoid");
	}else{
			...
			if(data.errorCode === '0'){
				if(data.resultMsg === "success"){//操作成功
					if("open" === obj.attr("saleSwitch")){
						...
						obj.parent().next().next().find("a").attr("statusname","已上架");
						obj.parent().next().next().next().find("a").attr("statusname","已上架");
						...
						obj.parent().next().next().find("a").attr("statusname","已下架");
						obj.parent().next().next().next().find("a").attr("statusname","已下架");
						$("#appInfoStatus" + obj.attr("appinfoid")).hide();
						$("#appInfoStatus" + obj.attr("appinfoid")).slideDown(300);
					}
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href="app/toView/"+ obj.attr("appinfoid");
});
